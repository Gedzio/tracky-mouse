...
TrackyMouse.init = function (div, { statsJs = false } = {}) {
	...
	uiContainer.innerHTML = `
		<div class="tracky-mouse-controls">
			...
			<br>
			${window.PostureMonitor ? window.PostureMonitor.getHTML() : ''}
			<br>
		</div>
	...
	// Posture monitor (UI, state, detection) lives in posture-monitor.js.
	// `setOptions` is declared later in this function; the closure delays the read until
	// the user actually triggers a setting change, by which time `setOptions` is initialized.
	if (window.PostureMonitor) {
		window.PostureMonitor.init({
			container: uiContainer,
			setOptions: function (o) { setOptions(o); },
		});
	}
	...
	function deserializeSettings(settings, initialLoad = false) {
		...
		if ("globalSettings" in settings) {
			...
			if (window.PostureMonitor) {
				window.PostureMonitor.loadSettings(settings.globalSettings);
			}
		}
	}
	...
	function serializeSettings() {
		...
		return {
			...
			globalSettings: {
				...
				toggleShortcut,
				postureMonitor: window.PostureMonitor ? window.PostureMonitor.getSettings() : undefined,
				// TODO:
	...
	// Posture settings IO, input wiring, and all button handlers now live inside posture-monitor.js.
	...
	function draw(update = true) {
		...
		if (window.PostureMonitor) {
			window.PostureMonitor.update(facemeshPrediction);
		}
	...
	// Always update (run facemesh even when tracking is paused) — posture monitor needs fresh
	// frames regardless of whether the mouse cursor is being moved.
